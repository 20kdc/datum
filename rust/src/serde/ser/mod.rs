...
impl<'write> PlainSerializer<'write> {
    ...
    pub fn new(target: &'write mut dyn Write) -> Self {
        Self {
            target,
    ...
    fn write_token<B: Deref<Target = str>>(&mut self, token: DatumToken<B>) -> error::Result<()> {
    ...
    fn write_atom<B: Deref<Target = str>>(&mut self, token: DatumAtom<B>) -> error::Result<()> {
        self.writer
    ...
    fn fmt_open_block(&mut self) -> error::Result<()> {
    ...
    fn fmt_close_block(&mut self) -> error::Result<()> {
        if self.style == Style::Indented {
            self.writer.indent -= 1;
        }
        Ok(())
    ...
    fn fmt_seq_newline(&mut self) -> error::Result<()> {
        if self.style == Style::Indented {
            self.writer
                .write_newline(self.target)
                .map_err(|e| error::Error::custom(e))?;
