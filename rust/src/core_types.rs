...
macro_rules! datum_error {
    ($kind:ident, $offset:expr, $info:literal) => {
        DatumError { kind: $crate::DatumErrorKind::$kind, offset: $offset, message: $info }
    };
}
...
macro_rules! datum_error {
    ($kind:ident, $offset:expr, $info:literal) => {
        DatumError { kind: $crate::DatumErrorKind::$kind, offset: $offset, message: "" }
    };
}
...
impl<I, V: Iterator<Item = I> + Sized> ViaDatumPipe<I> for V {
    fn via_datum_pipe<P: DatumPipe<Input = I>>(self, pipe: P) -> DatumPipeIterator<Self, I, P> {
        DatumPipeIterator { offset: 0, iterator: self, pipeline: pipe, buffer: VecDeque::new(), eof: false }
    }
}
